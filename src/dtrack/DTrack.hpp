/* DTrackSDK: C++ header file, A.R.T. GmbH 21.4.05-15.10.08
 *
 ...
 *
 * $Id: DTrack.hpp,v 1.7 2008/10/15 16:40:19 kurt Exp $
 */
...
// return value (o): boolean
	...
	bool timeout(void);     // timeout while receiving tracking data
	bool udperror(void);    // network error while receiving tracking data
	bool parseerror(void);  // error while parsing tracking data
...
private:
	...
	enum{ err_none, err_timeout, err_net, err_parse } d_lastdataerror;  // last data error
	...
	bool cmd_send(int cmd, int val = 0);  // send remote control command to DTrack
};
