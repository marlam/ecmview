/* DTrackSDK: C++ source file, A.R.T. GmbH 21.4.05-15.10.08
 *
 ...
 *
 * $Id: DTrack.cpp,v 1.7 2008/10/15 16:39:40 kurt Exp $
 */
...
static int udp_send(const void* sock, void* buffer, int len, unsigned int ipaddr, unsigned short port, int tout_us);
...
// Constructor:
//
// data_port (i): UDP port number to receive data from DTrack
//
...
// data_timeout_us (i): UDP timeout (receiving and sending) in us (micro second)
...
)
{
	...
	d_udpport = (unsigned short )data_port;
	...
	if(remote_host != NULL && remote_port != 0){
		...
		d_remote_ip = ip;
		d_remote_port = (unsigned short )remote_port;
	}else{
		d_remote_ip = 0;
		d_remote_port = 0;
	}
...
bool DTrack::timeout(void)         // timeout while receiving tracking data
{
	return (d_lastdataerror == err_timeout);
}

bool DTrack::udperror(void)        // network error while receiving tracking data
{
	return (d_lastdataerror == err_net);
}

bool DTrack::parseerror(void)      // error while parsing tracking data
{
...
void DTrack::set_data_noerror(void)     // 'no error'
{
	d_lastdataerror = err_none;
}

void DTrack::set_data_timeout(void)     // 'timeout'
{
	d_lastdataerror = err_timeout;
}

void DTrack::set_data_neterror(void)    // 'network error'
{
	d_lastdataerror = err_net;
}

void DTrack::set_data_parseerror(void)  // 'parse error'
{
