...
void main()
{
    ...
    float result = 0.0;
    if (ytex < 0.5)	// Approach
    {
	for (int r = 0; r < 6; r++)
	{
	    result += a_coeff[r] * texture2D(tex, vec2(xtex, 2.0 * ytex + float(r) * ystep)).g;
	}
    }
    else		// Details
    {
	for (int r = 0; r < 6; r++)
	{
	    result += d_coeff[r] * texture2D(tex, vec2(xtex, 2.0 * (ytex - 0.5) + float(r) * ystep)).g;
	}
    }
